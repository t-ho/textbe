...
public class StDebugThread extends org.be.textbe.st.textst.resource.st.debug.StDebugElement implements org.eclipse.debug.core.model.IThread, org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener {
	
	private boolean suspended = false;
	private org.be.textbe.st.textst.resource.st.debug.StDebugTarget debugTarget;
	
	public StDebugThread(org.be.textbe.st.textst.resource.st.debug.StDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.be.textbe.st.textst.resource.st.debug.StDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.be.textbe.st.textst.resource.st.util.StStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.be.textbe.st.textst.resource.st.debug.StStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.be.textbe.st.textst.resource.st.debug.StDebugTarget getTarget() {
		return debugTarget;
	}
