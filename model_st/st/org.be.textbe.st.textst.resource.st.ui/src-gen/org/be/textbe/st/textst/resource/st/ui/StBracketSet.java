...
public class StBracketSet {
	...
	private java.util.ArrayList<org.be.textbe.st.textst.resource.st.IStBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements org.be.textbe.st.textst.resource.st.IStBracketPair {
	 ...
	 */
	private class ClosingListener implements org.be.textbe.st.textst.resource.st.ui.IStBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				org.be.textbe.st.textst.resource.st.IStBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public StBracketSet(org.be.textbe.st.textst.resource.st.ui.StEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new org.be.textbe.st.textst.resource.st.mopp.StMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.IStBracketPair>();
	 ...
	 */
	public org.be.textbe.st.textst.resource.st.IStBracketPair getBracketPair(String opening, String closing) {
		for (org.be.textbe.st.textst.resource.st.IStBracketPair bracketPair : bracketPairs) {
	...
	public org.be.textbe.st.textst.resource.st.IStBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(org.be.textbe.st.textst.resource.st.IStBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + org.be.textbe.st.textst.resource.st.ui.StPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public org.be.textbe.st.textst.resource.st.IStBracketPair remove(String opening, String closing) {
		for (org.be.textbe.st.textst.resource.st.IStBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.IStBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
