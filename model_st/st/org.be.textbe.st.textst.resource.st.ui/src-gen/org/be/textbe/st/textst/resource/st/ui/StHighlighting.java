 ...
 */
public class StHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static org.be.textbe.st.textst.resource.st.ui.StPositionHelper positionHelper = new org.be.textbe.st.textst.resource.st.ui.StPositionHelper();
	...
	private org.be.textbe.st.textst.resource.st.ui.StTokenScanner scanner;
	...
	private org.be.textbe.st.textst.resource.st.ui.StEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private org.be.textbe.st.textst.resource.st.ui.StOccurrence occurrence;
	private org.be.textbe.st.textst.resource.st.ui.StBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, org.be.textbe.st.textst.resource.st.IStBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public StHighlighting(org.be.textbe.st.textst.resource.st.IStTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, org.be.textbe.st.textst.resource.st.ui.StColorManager colorManager, org.be.textbe.st.textst.resource.st.ui.StEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = org.be.textbe.st.textst.resource.st.ui.StUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new org.be.textbe.st.textst.resource.st.ui.StTokenScanner(textResource, colorManager);
		occurrence = new org.be.textbe.st.textst.resource.st.ui.StOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new org.be.textbe.st.textst.resource.st.ui.StBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(org.be.textbe.st.textst.resource.st.ui.StPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, org.be.textbe.st.textst.resource.st.ui.StPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(org.be.textbe.st.textst.resource.st.ui.StEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(org.be.textbe.st.textst.resource.st.ui.StPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new org.be.textbe.st.textst.resource.st.ui.StEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
