...
public class CtDebugThread extends org.be.textbe.ct.textct.resource.ct.debug.CtDebugElement implements org.eclipse.debug.core.model.IThread, org.be.textbe.ct.textct.resource.ct.debug.ICtDebugEventListener {
	
	private boolean suspended = false;
	private org.be.textbe.ct.textct.resource.ct.debug.CtDebugTarget debugTarget;
	
	public CtDebugThread(org.be.textbe.ct.textct.resource.ct.debug.CtDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.be.textbe.ct.textct.resource.ct.debug.CtDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.be.textbe.ct.textct.resource.ct.util.CtStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.be.textbe.ct.textct.resource.ct.debug.CtStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.be.textbe.ct.textct.resource.ct.debug.CtDebugTarget getTarget() {
		return debugTarget;
	}
