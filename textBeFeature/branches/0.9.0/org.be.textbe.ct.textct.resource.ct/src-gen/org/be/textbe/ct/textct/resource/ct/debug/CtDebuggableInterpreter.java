 ...
 */
public class CtDebuggableInterpreter<ResultType, ContextType> extends org.be.textbe.ct.textct.resource.ct.debug.AbstractCtDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.be.textbe.ct.textct.resource.ct.util.AbstractCtInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.be.textbe.ct.textct.resource.ct.ICtCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public CtDebuggableInterpreter(org.be.textbe.ct.textct.resource.ct.util.AbstractCtInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.be.textbe.ct.textct.resource.ct.ICtInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.be.textbe.ct.textct.resource.ct.util.CtStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.be.textbe.ct.textct.resource.ct.util.AbstractCtInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.be.textbe.ct.textct.resource.ct.ICtLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.be.textbe.ct.textct.resource.ct.ICtLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof org.be.textbe.ct.textct.resource.ct.ICtTextResource) {
			org.be.textbe.ct.textct.resource.ct.ICtTextResource textResource = (org.be.textbe.ct.textct.resource.ct.ICtTextResource) resource;
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.be.textbe.ct.textct.resource.ct.ICtCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.be.textbe.ct.textct.resource.ct.debug.ECtDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.be.textbe.ct.textct.resource.ct.util.CtEObjectUtil.getDepth(current);
		stopCondition = new org.be.textbe.ct.textct.resource.ct.ICtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.be.textbe.ct.textct.resource.ct.util.CtEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.be.textbe.ct.textct.resource.ct.util.CtEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.be.textbe.ct.textct.resource.ct.ICtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
