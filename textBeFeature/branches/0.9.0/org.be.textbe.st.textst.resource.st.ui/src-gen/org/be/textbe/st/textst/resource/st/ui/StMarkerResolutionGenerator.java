...
public class StMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(org.be.textbe.st.textst.resource.st.IStOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof org.be.textbe.st.textst.resource.st.mopp.StResource) {
					org.be.textbe.st.textst.resource.st.mopp.StResource customResource = (org.be.textbe.st.textst.resource.st.mopp.StResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<org.be.textbe.st.textst.resource.st.IStQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final org.be.textbe.st.textst.resource.st.IStQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									org.be.textbe.st.textst.resource.st.ui.StUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			org.be.textbe.st.textst.resource.st.ui.StUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<org.be.textbe.st.textst.resource.st.IStQuickFix> getQuickFixes(org.be.textbe.st.textst.resource.st.IStTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					org.be.textbe.st.textst.resource.st.IStQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
