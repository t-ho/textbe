...
public class StDebugTarget extends org.be.textbe.st.textst.resource.st.debug.StDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.be.textbe.st.textst.resource.st.debug.StDebugProcess process;
	...
	private org.be.textbe.st.textst.resource.st.debug.StDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener> eventListeners = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener>();
	
	private org.be.textbe.st.textst.resource.st.debug.StDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.be.textbe.st.textst.resource.st.mopp.StMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.be.textbe.st.textst.resource.st.debug.StDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.be.textbe.st.textst.resource.st.debug.StDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public StDebugTarget(org.be.textbe.st.textst.resource.st.debug.StDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.be.textbe.st.textst.resource.st.debug.StDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.be.textbe.st.textst.resource.st.debug.StDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.be.textbe.st.textst.resource.st.debug.IStDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.be.textbe.st.textst.resource.st.mopp.StMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.be.textbe.st.textst.resource.st.debug.StLineBreakpoint lineBreakpoint = (org.be.textbe.st.textst.resource.st.debug.StLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.be.textbe.st.textst.resource.st.debug.StDebugMessage message) {
		try {
			if (message.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.RESUMED)) {
	...
	public org.be.textbe.st.textst.resource.st.debug.StDebugProxy getDebugProxy() {
