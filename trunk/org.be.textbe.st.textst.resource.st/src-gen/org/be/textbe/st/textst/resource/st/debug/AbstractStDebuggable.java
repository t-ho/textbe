...
public abstract class AbstractStDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.be.textbe.st.textst.resource.st.util.StPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.util.StPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			org.be.textbe.st.textst.resource.st.mopp.StPlugin.logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			org.be.textbe.st.textst.resource.st.mopp.StPlugin.logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.be.textbe.st.textst.resource.st.debug.StDebugMessage message = new org.be.textbe.st.textst.resource.st.debug.StDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.be.textbe.st.textst.resource.st.util.StPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.be.textbe.st.textst.resource.st.util.StPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.be.textbe.st.textst.resource.st.util.StPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.STARTED, true);
			setSuspend(true);
