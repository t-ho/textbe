 ...
 * </ul>
 */
public class CtBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new org.be.textbe.ct.textct.resource.ct.mopp.CtMetaInformation().getSyntaxName();
	
	private org.be.textbe.ct.textct.resource.ct.ui.CtBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public CtBracketPreferencePage() {
		super();
		
		org.be.textbe.ct.textct.resource.ct.ICtMetaInformation metaInformation = new org.be.textbe.ct.textct.resource.ct.mopp.CtMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(org.be.textbe.ct.textct.resource.ct.ui.CtUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new org.be.textbe.ct.textct.resource.ct.ui.CtBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					org.be.textbe.ct.textct.resource.ct.ICtBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					org.be.textbe.ct.textct.resource.ct.ICtBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + org.be.textbe.ct.textct.resource.ct.ui.CtPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof org.be.textbe.ct.textct.resource.ct.ui.CtEditor) {
			((org.be.textbe.ct.textct.resource.ct.ui.CtEditor) editor).invalidateTextRepresentation();
		}
	}
