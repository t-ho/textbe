	...
	while(cInputPos.IsNotNull()){
		...
		if (strToken1 != _T("")){
			...
			if (strDataInInput == _T("")){
				...
				strMessage = strMessage + _T("is sending data.");
				CTranslateException cException(strMessage);
				throw cException;
			}
			...
			if (pcNode->IsUserDefinedAttribute()){  // Remember that at the moment, it has to be that either both
				...
				strRightSide = strComponentName + _T("_") + strFirstData;
			...
			strAction = strLeftSide + _T("'=") + strRightSide;
			plActions->AddTail(strAction);
			if (strSecondData != _T("")){
				if (strSecondDataInInput == _T("")){
					...
					strMessage = strMessage + strComponentName + _T(" <") + strStateName + _T(">\r\n");
					strMessage = strMessage + _T("is sending two data items.");
					CTranslateException cException(strMessage);
					throw cException;
				}
				...
				if (pcNode->IsUserDefinedAttribute()){
					...
					strSecondRightSide = strComponentName + _T("_") + strSecondData;
				...
				strAction = strSecondLeftSide + _T("'=") + strSecondRightSide;
				plActions->AddTail(strAction);
			}else{
				...
				if (strSecondDataInInput != _T("")){
					...
					strMessage = strMessage + strComponentName + _T(" <") + strStateName + _T(">\r\n");
					strMessage = strMessage + _T("is sending only one data item.");
					CTranslateException cException(strMessage);
					throw cException;
				}
			}
