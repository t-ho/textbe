...
void CTranslateSALMain::SelectOptions(const wchar_t* strTree, int iPriority, int iBuff, int iWithSets, const wchar_t* strSets, const wchar_t* strInit)
{
	...
	//	if (bUsingSets){
		...
		//	
				...
				if (bUsingSets){ // This means that it has set nodes.
					if (m_strSets == _T("")){
						NString strMessage = _T("You have not provided any information about the sets. ");
						CTranslateException cException(strMessage);
						throw cException;
					}
...
/////////// Do something if it is 0 as an error occurred.
	...
	//				_tfopen_s(&pcFile,(LPCTSTR)m_strPathName2,_T("rt"));
				...
				//		strLine = cLine;
						strLine = m_strInit.Tokenize(_T("\n"), iFilePos);
						if (strLine.Trim() != _T("")){ // To make it ignore the last blank line.
