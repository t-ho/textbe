...
public class BtDebugThread extends org.be.textbe.bt.v1.textbt.resource.bt.debug.BtDebugElement implements org.eclipse.debug.core.model.IThread, org.be.textbe.bt.v1.textbt.resource.bt.debug.IBtDebugEventListener {
	
	private boolean suspended = false;
	private org.be.textbe.bt.v1.textbt.resource.bt.debug.BtDebugTarget debugTarget;
	
	public BtDebugThread(org.be.textbe.bt.v1.textbt.resource.bt.debug.BtDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.be.textbe.bt.v1.textbt.resource.bt.debug.BtDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.be.textbe.bt.v1.textbt.resource.bt.util.BtStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.be.textbe.bt.v1.textbt.resource.bt.debug.BtStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.be.textbe.bt.v1.textbt.resource.bt.debug.BtDebugTarget getTarget() {
		return debugTarget;
	}
