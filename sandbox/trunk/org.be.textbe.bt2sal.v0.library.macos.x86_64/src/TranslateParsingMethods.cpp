...
void CTranslateParsingMethods::StoreNodeTransition(CTranslateSALMain& cTranslateMain, int iNode, NString strGuard, NList<NString, NString>* plActions, int iParent, bool bIsExternal, bool bIsInternalInput)
{
	...
	if (cTranslateMain.IsAtomic(iNode)){
		...
		if (bIsBottom){  
			...
			if (strPreviousGuard != ""){
				if (strGuard != ""){
					strGuard = strPreviousGuard + " AND " + strGuard;
				}else{
			...
			NString strFullGuard = "";
			NString strFullOppositeGuard = "";  // The NOT case for selection nodes.
			...
			if (cTranslateMain.UsingViews()){
				...
				while (cAtomicPosition.IsNotNull()){
					...
					if (strTempGuard != ""){
						if (pcCurrentNode->GetType() == GSE_T_CONDITION){
							...
							if (strFullOppositeGuard == ""){
								strFullOppositeGuard = "NOT(" + strTempGuard + ")";
								// Add the rest of the normal guard created so far.
								if (strFullGuard != ""){
									strFullOppositeGuard.Append(" AND ");
									strFullOppositeGuard.Append(strFullGuard);
								}
							}else{
								strFullOppositeGuard.Append(" AND NOT(");
								strFullOppositeGuard.Append(strTempGuard);
								strFullOppositeGuard.Append(")");
							}
							if (strFullGuard != ""){
								strFullGuard = strTempGuard + " AND " + strFullGuard;
							}else{
				...
				if (iProgramCounter == iTopPC){ // Since both have the same pc, this block
					...
					if (strFullGuard != ""){
						strFullGuard =  " AND " + strFullGuard;
						strFullGuard.Format("%d" + strFullGuard, iProgramCounterValue);
						strFullGuard = strProgramCounterName + "=" + strFullGuard;
					}else{
						strFullGuard = strProgramCounterName + "=";
						strFullGuard.Format(strFullGuard + "%d", iProgramCounterValue);
					}

					if (strFullOppositeGuard != ""){
						strFullOppositeGuard =  " AND " + strFullOppositeGuard;
						strFullOppositeGuard.Format("%d" + strFullOppositeGuard, iProgramCounterValue);
						strFullOppositeGuard = strProgramCounterName + "=" + strFullOppositeGuard;
					}
				}
				NString strAction = strProgramCounterName + "'=";
				strAction.Format(strAction + "%d", iNewValue);
				plFullActions->AddTail(strAction);
	
				if (cTranslateMain.GetTranslationType() == 4){
					if (pcBlock->ContainsExternal() == true){
						strFullGuard = strFullGuard + " AND NOT(internalActions)"; 
					}else if (pcBlock->ContainsInternalInput() != true){
						strFullGuard = strFullGuard + " AND NOT(messageReady)"; 
					}
				}
				...
				if (iGuardPosition == -1){
					if (strFullOppositeGuard != ""){
						...
						while (cPCPos.IsNotNull()){
							...
							strResetAction = "pc";
							strResetAction.Format(strResetAction + "%d", iCurrentPC);
							strResetAction.Append("'=0");
	...
	//		}
		...
		// Not atomic.
		if(strGuard == ""){
			strGuard = strProgramCounterName + "=";
			strGuard.Format(strGuard + "%d", iProgramCounterValue);
		}else{
			strGuard =  " AND " + strGuard;
			strGuard.Format("%d" + strGuard, iProgramCounterValue);
			strGuard = strProgramCounterName + "=" + strGuard;
		}
		NString strAction = strProgramCounterName + "'=";
		strAction.Format(strAction + "%d", iNewValue);
		...
		if (cTranslateMain.GetTranslationType() == 4){
			if (bIsExternal == true){
				strGuard = strGuard + " AND NOT(internalActions)"; 
			}else if (!bIsInternalInput){
				strGuard = strGuard + " AND NOT(messageReady)"; 
			}
		}
