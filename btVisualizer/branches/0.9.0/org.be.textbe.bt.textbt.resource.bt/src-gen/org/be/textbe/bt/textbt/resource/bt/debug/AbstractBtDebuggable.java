...
public abstract class AbstractBtDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.be.textbe.bt.textbt.resource.bt.util.BtPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.be.textbe.bt.textbt.resource.bt.util.BtPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			org.be.textbe.bt.textbt.resource.bt.mopp.BtPlugin.logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			org.be.textbe.bt.textbt.resource.bt.mopp.BtPlugin.logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.be.textbe.bt.textbt.resource.bt.debug.EBtDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.be.textbe.bt.textbt.resource.bt.debug.BtDebugMessage message = new org.be.textbe.bt.textbt.resource.bt.debug.BtDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.be.textbe.bt.textbt.resource.bt.util.BtPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.be.textbe.bt.textbt.resource.bt.debug.EBtDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.be.textbe.bt.textbt.resource.bt.debug.EBtDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.be.textbe.bt.textbt.resource.bt.util.BtPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.be.textbe.bt.textbt.resource.bt.util.BtPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.be.textbe.bt.textbt.resource.bt.debug.EBtDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.be.textbe.bt.textbt.resource.bt.debug.EBtDebugMessageTypes.STARTED, true);
			setSuspend(true);
