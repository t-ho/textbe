...
public class CtOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "org.be.textbe.ct.textct.resource.ct.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.be.textbe.ct.textct.resource.ct.ui.occurences.declaration";
	...
	private org.be.textbe.ct.textct.resource.ct.ui.CtTokenScanner tokenScanner;
	 ...
	 */
	public CtOccurrence(org.be.textbe.ct.textct.resource.ct.ICtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.be.textbe.ct.textct.resource.ct.ui.CtTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.be.textbe.ct.textct.resource.ct.mopp.CtMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.be.textbe.ct.textct.resource.ct.ui.CtBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.be.textbe.ct.textct.resource.ct.ui.CtPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.be.textbe.ct.textct.resource.ct.ui.CtPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.be.textbe.ct.textct.resource.ct.ui.CtPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.be.textbe.ct.textct.resource.ct.ui.CtPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.be.textbe.ct.textct.resource.ct.ui.CtOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.be.textbe.ct.textct.resource.ct.ui.CtOccurrence.DECLARATION_ANNOTATION_ID);
	}
