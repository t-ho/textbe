 ...
 * contains <code>org.be.textbe.st.textst.resource.st.IStTextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class StMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = org.be.textbe.st.textst.resource.st.mopp.StPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<org.be.textbe.st.textst.resource.st.IStCommand<Object>> commands = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.IStCommand<Object>>();
		private final Object jobLock = new Object();
		
		public void addCommand(org.be.textbe.st.textst.resource.st.IStCommand<Object> command) {
			...
			new org.eclipse.core.runtime.jobs.Job("updating markers") {
				...
				protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
					synchronized(jobLock) {
						java.util.List<org.be.textbe.st.textst.resource.st.IStCommand<Object>> commandsToProcess = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.IStCommand<Object>>();
						synchronized(commands) {
							commandsToProcess.addAll(commands);
							commands.clear();
						}
						for (org.be.textbe.st.textst.resource.st.IStCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final org.be.textbe.st.textst.resource.st.IStTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final org.be.textbe.st.textst.resource.st.IStTextDiagnostic diagnostic) {
		final org.be.textbe.st.textst.resource.st.IStProblem problem = diagnostic.getProblem();
		org.be.textbe.st.textst.resource.st.StEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new org.be.textbe.st.textst.resource.st.IStCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == org.be.textbe.st.textst.resource.st.StEProblemSeverity.ERROR) {
					...
					org.be.textbe.st.textst.resource.st.IStTextDiagnostic textDiagnostic = (org.be.textbe.st.textst.resource.st.IStTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof org.be.textbe.st.textst.resource.st.mopp.StResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((org.be.textbe.st.textst.resource.st.mopp.StResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<org.be.textbe.st.textst.resource.st.IStQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, org.be.textbe.st.textst.resource.st.util.StStringUtil.explode(sourceIDs, "|"));
					}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (org.be.textbe.st.textst.resource.st.StEProblemType nextType : org.be.textbe.st.textst.resource.st.StEProblemType.values()) {
	 ...
	 */
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, org.be.textbe.st.textst.resource.st.StEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(org.be.textbe.st.textst.resource.st.StEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	private static String getMarkerID(org.be.textbe.st.textst.resource.st.StEProblemType problemType) {
	...
	private static void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			org.be.textbe.st.textst.resource.st.mopp.StPlugin.logError("Error while removing markers from resource:", ce);
		}
	}
}
