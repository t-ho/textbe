...
/**
 * A text editor for 'st' models.
 */
public class StEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, org.be.textbe.st.textst.resource.st.IStResourceProvider, org.be.textbe.st.textst.resource.st.ui.IStBracketHandlerProvider, org.be.textbe.st.textst.resource.st.ui.IStAnnotationModelProvider {
	
	private org.be.textbe.st.textst.resource.st.ui.StHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private org.be.textbe.st.textst.resource.st.ui.StCodeFoldingManager codeFoldingManager;
	private org.be.textbe.st.textst.resource.st.ui.StBackgroundParsingStrategy bgParsingStrategy = new org.be.textbe.st.textst.resource.st.ui.StBackgroundParsingStrategy();
	private java.util.Collection<org.be.textbe.st.textst.resource.st.IStBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<org.be.textbe.st.textst.resource.st.IStBackgroundParsingListener>();
	private org.be.textbe.st.textst.resource.st.ui.StColorManager colorManager = new org.be.textbe.st.textst.resource.st.ui.StColorManager();
	private org.be.textbe.st.textst.resource.st.ui.StOutlinePage outlinePage;
	...
	private org.be.textbe.st.textst.resource.st.ui.StPropertySheetPage propertySheetPage;
	...
	private org.be.textbe.st.textst.resource.st.ui.IStBracketHandler bracketHandler;
	...
	public StEditor() {
		super();
		setSourceViewerConfiguration(new org.be.textbe.st.textst.resource.st.ui.StEditorConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), StEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								org.be.textbe.st.textst.resource.st.IStTextResource currentResource = getResource();
				...
				org.be.textbe.st.textst.resource.st.ui.StUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("org.be.textbe.st.textst.resource.st.EditorContext");
		setRulerContextMenuId("org.be.textbe.st.textst.resource.st.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new org.be.textbe.st.textst.resource.st.ui.StHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new org.be.textbe.st.textst.resource.st.ui.StCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		org.be.textbe.st.textst.resource.st.mopp.StNature.activate(inputFile.getProject());
		...
		org.be.textbe.st.textst.resource.st.IStTextResource loadedResource = (org.be.textbe.st.textst.resource.st.IStTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				org.be.textbe.st.textst.resource.st.IStTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof org.be.textbe.st.textst.resource.st.IStTextResource) {
					setResource((org.be.textbe.st.textst.resource.st.IStTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					org.be.textbe.st.textst.resource.st.ui.StUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the StEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				org.be.textbe.st.textst.resource.st.ui.StUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(org.be.textbe.st.textst.resource.st.IStTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new org.be.textbe.st.textst.resource.st.ui.StOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new org.be.textbe.st.textst.resource.st.ui.StPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			org.be.textbe.st.textst.resource.st.IStTextResource textResource = (org.be.textbe.st.textst.resource.st.IStTextResource) element.eResource();
			org.be.textbe.st.textst.resource.st.IStLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			org.be.textbe.st.textst.resource.st.IStTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				org.be.textbe.st.textst.resource.st.IStTextToken token = lexer.getNextToken();
			...
			org.be.textbe.st.textst.resource.st.ui.StUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!StEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(org.be.textbe.st.textst.resource.st.IStBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (org.be.textbe.st.textst.resource.st.IStBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public org.be.textbe.st.textst.resource.st.ui.IStBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(org.be.textbe.st.textst.resource.st.ui.IStBracketHandler bracketHandler) {
