 ...
 */
public class StOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.be.textbe.st.textst.resource.st.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.be.textbe.st.textst.resource.st.ui.occurences.declaration";
	...
	private org.be.textbe.st.textst.resource.st.ui.StTokenScanner tokenScanner;
	...
	private org.be.textbe.st.textst.resource.st.IStTextResource textResource;
	 ...
	 */
	public StOccurrence(org.be.textbe.st.textst.resource.st.IStTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.be.textbe.st.textst.resource.st.ui.StTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.be.textbe.st.textst.resource.st.mopp.StMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.be.textbe.st.textst.resource.st.ui.StBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.be.textbe.st.textst.resource.st.ui.StPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.be.textbe.st.textst.resource.st.ui.StPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.be.textbe.st.textst.resource.st.ui.StOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.be.textbe.st.textst.resource.st.ui.StOccurrence.DECLARATION_ANNOTATION_ID);
	}
