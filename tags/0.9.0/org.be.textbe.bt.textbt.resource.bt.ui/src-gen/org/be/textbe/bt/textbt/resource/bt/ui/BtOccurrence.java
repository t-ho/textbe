...
public class BtOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "org.be.textbe.bt.textbt.resource.bt.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.be.textbe.bt.textbt.resource.bt.ui.occurences.declaration";
	...
	private org.be.textbe.bt.textbt.resource.bt.ui.BtTokenScanner tokenScanner;
	...
	private org.be.textbe.bt.textbt.resource.bt.IBtTextResource textResource;
	 ...
	 */
	public BtOccurrence(org.be.textbe.bt.textbt.resource.bt.IBtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.be.textbe.bt.textbt.resource.bt.ui.BtTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.be.textbe.bt.textbt.resource.bt.mopp.BtMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.be.textbe.bt.textbt.resource.bt.ui.BtBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.be.textbe.bt.textbt.resource.bt.ui.BtPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.be.textbe.bt.textbt.resource.bt.ui.BtPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.be.textbe.bt.textbt.resource.bt.ui.BtPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.be.textbe.bt.textbt.resource.bt.ui.BtPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.be.textbe.bt.textbt.resource.bt.ui.BtOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.be.textbe.bt.textbt.resource.bt.ui.BtOccurrence.DECLARATION_ANNOTATION_ID);
	}
