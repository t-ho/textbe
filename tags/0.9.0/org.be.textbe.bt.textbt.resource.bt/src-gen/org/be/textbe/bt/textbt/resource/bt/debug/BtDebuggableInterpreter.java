 ...
 */
public class BtDebuggableInterpreter<ResultType, ContextType> extends org.be.textbe.bt.textbt.resource.bt.debug.AbstractBtDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.be.textbe.bt.textbt.resource.bt.util.AbstractBtInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.be.textbe.bt.textbt.resource.bt.IBtCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public BtDebuggableInterpreter(org.be.textbe.bt.textbt.resource.bt.util.AbstractBtInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.be.textbe.bt.textbt.resource.bt.IBtInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.be.textbe.bt.textbt.resource.bt.util.BtStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.be.textbe.bt.textbt.resource.bt.util.AbstractBtInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.be.textbe.bt.textbt.resource.bt.IBtLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.be.textbe.bt.textbt.resource.bt.IBtLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.be.textbe.bt.textbt.resource.bt.IBtCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.be.textbe.bt.textbt.resource.bt.debug.EBtDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.be.textbe.bt.textbt.resource.bt.util.BtEObjectUtil.getDepth(current);
		stopCondition = new org.be.textbe.bt.textbt.resource.bt.IBtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.be.textbe.bt.textbt.resource.bt.util.BtEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.be.textbe.bt.textbt.resource.bt.util.BtEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.be.textbe.bt.textbt.resource.bt.IBtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
