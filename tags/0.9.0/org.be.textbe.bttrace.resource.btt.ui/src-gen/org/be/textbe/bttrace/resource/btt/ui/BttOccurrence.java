 ...
 */
public class BttOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.be.textbe.bttrace.resource.btt.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.be.textbe.bttrace.resource.btt.ui.occurences.declaration";
	...
	private org.be.textbe.bttrace.resource.btt.ui.BttTokenScanner tokenScanner;
	...
	private org.be.textbe.bttrace.resource.btt.IBttTextResource textResource;
	 ...
	 */
	public BttOccurrence(org.be.textbe.bttrace.resource.btt.IBttTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.be.textbe.bttrace.resource.btt.ui.BttTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.be.textbe.bttrace.resource.btt.mopp.BttMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.be.textbe.bttrace.resource.btt.ui.BttBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.be.textbe.bttrace.resource.btt.ui.BttPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.be.textbe.bttrace.resource.btt.ui.BttPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.be.textbe.bttrace.resource.btt.ui.BttPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.be.textbe.bttrace.resource.btt.ui.BttPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.be.textbe.bttrace.resource.btt.ui.BttOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.be.textbe.bttrace.resource.btt.ui.BttOccurrence.DECLARATION_ANNOTATION_ID);
	}
