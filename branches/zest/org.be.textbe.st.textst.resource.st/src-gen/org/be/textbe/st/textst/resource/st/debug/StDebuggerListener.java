 ...
 * code).
 */
public class StDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private org.be.textbe.st.textst.resource.st.debug.AbstractStDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, org.be.textbe.st.textst.resource.st.util.StPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, org.be.textbe.st.textst.resource.st.util.StPair<String, Object>>();
	...
	public StDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		org.be.textbe.st.textst.resource.st.debug.StDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = org.be.textbe.st.textst.resource.st.util.StStringUtil.encode('#', stack);
				org.be.textbe.st.textst.resource.st.debug.StDebugMessage message = new org.be.textbe.st.textst.resource.st.debug.StDebugMessage(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				org.be.textbe.st.textst.resource.st.debug.StDebugMessage message = new org.be.textbe.st.textst.resource.st.debug.StDebugMessage(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.GET_VARIABLES)) {
				...
				org.be.textbe.st.textst.resource.st.debug.StDebugMessage message = new org.be.textbe.st.textst.resource.st.debug.StDebugMessage(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return org.be.textbe.st.textst.resource.st.util.StStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		org.be.textbe.st.textst.resource.st.util.StPair<String, Object> pair = new org.be.textbe.st.textst.resource.st.util.StPair<String, Object>(name, value);
	...
	public org.be.textbe.st.textst.resource.st.debug.AbstractStDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(org.be.textbe.st.textst.resource.st.debug.AbstractStDebuggable debuggable) {
