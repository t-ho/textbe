 ...
 */
public class StDebuggableInterpreter<ResultType, ContextType> extends org.be.textbe.st.textst.resource.st.debug.AbstractStDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.be.textbe.st.textst.resource.st.util.AbstractStInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.be.textbe.st.textst.resource.st.IStCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public StDebuggableInterpreter(org.be.textbe.st.textst.resource.st.util.AbstractStInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.be.textbe.st.textst.resource.st.IStInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.be.textbe.st.textst.resource.st.util.StStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.be.textbe.st.textst.resource.st.util.AbstractStInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.be.textbe.st.textst.resource.st.IStLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.be.textbe.st.textst.resource.st.IStLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof org.be.textbe.st.textst.resource.st.IStTextResource) {
			org.be.textbe.st.textst.resource.st.IStTextResource textResource = (org.be.textbe.st.textst.resource.st.IStTextResource) resource;
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.be.textbe.st.textst.resource.st.IStCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.be.textbe.st.textst.resource.st.debug.EStDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.be.textbe.st.textst.resource.st.util.StEObjectUtil.getDepth(current);
		stopCondition = new org.be.textbe.st.textst.resource.st.IStCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.be.textbe.st.textst.resource.st.util.StEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.be.textbe.st.textst.resource.st.util.StEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.be.textbe.st.textst.resource.st.IStCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
